...
public class HelloController implements  Initializable {
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        ...
        if (anchorPane != null) {
            Pillars.addFirstPilar(pillars, anchorPane, 79, 86, 103);
            FactoryObject.generatePillars(pillars,pillarPosition,anchorPane);
            pillarPosition++;
            an.setmessage(0,Instruct);
            cherryInstance.generateCherry(pillars, pillarPosition, anchorPane, cherries);
            if (gameController.getGameState() == 1) {
                int l = gameController.getCherries() - 1;
                ...
                cherrycount.setText(String.valueOf(gameController.getCherries()));
                Score.setText("Score: " + gameController.getScore());
                Score.setStyle("-fx-font-weight: bold; -fx-font-size: 16; -fx-text-fill: #00008B;");
            } else {
    ...
    private void reviveGame(ActionEvent event) throws IOException {
        ...
        if (gameController.getCherries() >= 1) {
    ...
    public void testReviveGame() throws IOException {
        gameController.setCherries(1);
        helloController.reviveGame(new ActionEvent());
        assertEquals(2, gameController.getCherries());
    }

    @Test
    public void testResumeGame() throws IOException {
        gameController.setCherries(2);
        helloController.ResumeGame(new ActionEvent());
        assertEquals(1, gameController.getCherries());
    }
    ...
    public  void checkCharacterPosition() {
        if (pillars.size() > 1) {
            ...
            double next_X = pillars.get(pillarPosition).getBoundsInParent().getMinX();
            double pillarWidth = pillars.get(pillarPosition).getWidth();
            ...
            if (C_X < next_X || charMaxX > next_X + pillarWidth) {
                ...
                Animation.movePillarsAnimation(pillars, pillarPosition, Character, Stick, cherries);
                double characterPos = Character.getBoundsInParent().getMaxX();
                double maxnextPillarX = pillars.get(pillarPosition).getBoundsInParent().getMaxX();
